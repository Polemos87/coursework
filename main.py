...
class VKFotos():
    ...
    def get_photos(self):
        ...
        response = requests.get(photos, params={**self.params,
                                                **photos_params}).json()
...
class YAfotos():
    ...
    def create_folder(self):
        ...
        requests.put(f'{self.url}', params={'path': {name_folder}},
                      headers=self.headers)
        for photos in photos_dict:
            name_photo = str(photos)
            for key in photos_dict[photos]:
                response = requests.post(self.url + 'upload',
                                         headers=self.headers,
                                         params={'path': f'{name_folder}/'
                                                         f'{name_photo}.jpg',
                                                 'url':
                                                     photos_dict[photos][key]})

                if response.ok:
                    for i in tqdm(list):
                        time.sleep(1)
                        list_json.append({"file_name": f'{name_photo}.jpeg',
                                         "size": key})
                    else:
                        pass
...
token_vk = '958eb5d439726565e9333aa30e50e0f937ee43' \
           '2e927f0dbd541c541887d919a7c56f95c04217915c32008'
up = VKFotos(token_vk, '5.131')
